...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-1dc3830c";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-1dc3830c">'+'<span class="tile-face-1dc3830c tile-front-1dc3830c"></span>'+'<span class="tile-face-1dc3830c tile-back-1dc3830c"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-1dc3830c")){return;}
var back=tile.querySelector(".tile-back-1dc3830c");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-1dc3830c");if(isMine){tile.classList.add("boom-1dc3830c");}
if(dim){tile.classList.add("dimmed-1dc3830c");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-1dc3830c")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-1dc3830c");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-1dc3830c")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-1dc3830c");void panelEl.offsetWidth;panelEl.classList.add("jolt-1dc3830c");}
endRound();}
